...
# ===========================================================================
# Quadratic attention + Log-PE
# ===========================================================================
class MS_Attention_XNOR_GRAY_LogPE(nn.Module):
    """
    Fair ablation:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Gray2D: concatenate to binary q and k only
        gray = self.gray_pe.view(1, 1, 1, N, 2 * self.gray_bits).expand(T_, B, H, N, -1)
        q_ext = torch.cat([q, gray], dim=-1)   # (T, B, H, N, head_dim + 2*gray_bits)
        k_ext = torch.cat([k, gray], dim=-1)   # (T, B, H, N, head_dim + 2*gray_bits)


        D_ext = q_ext.size(-1)
        sum_q = q_ext.sum(dim=-1, keepdim=True)             # (T, B, H, N, 1)
        sum_k = k_ext.sum(dim=-1, keepdim=True).transpose(-2, -1)  # (T, B, H, 1, N)
        qk = torch.matmul(q_ext, k_ext.transpose(-2, -1))  # (T, B, H, N, N)
        attn = D_ext - sum_q - sum_k + 2 * qk
...
class MS_Block(nn.Module):
    def __init__(
        ...
        if attn_type == "quadratic_logpe":
            self.attn = MS_Attention_XNOR_GRAY_LogPE(
                dim=dim,
                num_heads=num_heads,
                sr_ratio=sr_ratio,
