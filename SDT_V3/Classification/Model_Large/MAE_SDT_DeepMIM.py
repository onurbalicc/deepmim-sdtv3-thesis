...
class SpikmaeDeepMIM(nn.Module):
    def __init__(self, T=1,choice=None,
        ...
        self.aux_decoder_embed6 = nn.Linear(embed_dim[-1], decoder_embed_dim, bias=True)
        self.aux_decoder_embed8 = nn.Linear(embed_dim[-1], decoder_embed_dim, bias=True)
        self.aux_decoder_embed10 = nn.Linear(embed_dim[-1], decoder_embed_dim, bias=True)

        self.aux_mask_token6 = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))
        self.aux_mask_token8 = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))
        self.aux_mask_token10 = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))

        self.aux_decoder_pos_embed6 = nn.Parameter(torch.zeros(1, num_patches, decoder_embed_dim), requires_grad=False)
        self.aux_decoder_pos_embed8 = nn.Parameter(torch.zeros(1, num_patches, decoder_embed_dim), requires_grad=False)
        self.aux_decoder_pos_embed10 = nn.Parameter(torch.zeros(1, num_patches, decoder_embed_dim), requires_grad=False)

        self.aux_decoder_blocks6 = nn.ModuleList([
            Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=False, norm_layer=norm_layer)
            for i in range(decoder_depth)
        ])
        self.aux_decoder_blocks8 = nn.ModuleList([
            Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=False, norm_layer=norm_layer)
            for i in range(decoder_depth)
        ])
        self.aux_decoder_blocks10 = nn.ModuleList([
            Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=False, norm_layer=norm_layer)
            for i in range(decoder_depth)
        ])

        self.aux_decoder_norm6 = norm_layer(decoder_embed_dim)
        self.aux_decoder_norm8 = norm_layer(decoder_embed_dim)
        self.aux_decoder_norm10 = norm_layer(decoder_embed_dim)

        self.aux_decoder_pred6 = nn.Linear(decoder_embed_dim, patch_size ** 2 * in_channels, bias=True)
        self.aux_decoder_pred8 = nn.Linear(decoder_embed_dim, patch_size ** 2 * in_channels, bias=True)
        self.aux_decoder_pred10 = nn.Linear(decoder_embed_dim, patch_size ** 2 * in_channels, bias=True)
    ...
    def initialize_weights(self):
        ...
        decoder_pos_embed_t = torch.from_numpy(decoder_pos_embed).float().unsqueeze(0)
        self.decoder_pos_embed.data.copy_(decoder_pos_embed_t)
        self.aux_decoder_pos_embed6.data.copy_(decoder_pos_embed_t)
        self.aux_decoder_pos_embed8.data.copy_(decoder_pos_embed_t)
        self.aux_decoder_pos_embed10.data.copy_(decoder_pos_embed_t)

        torch.nn.init.normal_(self.mask_token, std=.02)
        torch.nn.init.normal_(self.aux_mask_token6, std=.02)
        torch.nn.init.normal_(self.aux_mask_token8, std=.02)
        torch.nn.init.normal_(self.aux_mask_token10, std=.02)
    ...
    def forward_encoder(self, x , mask_ratio=1.0):
        ...
        feat12 = feat12.mean(0)
        x = self.norm(feat12).transpose(-1, -2).contiguous()
        return x, feat6, feat8, feat10, feat12, active, ids_restore, active_hw

    def forward_aux_decoder(self, x, ids_restore, decoder_embed, mask_token, decoder_pos_embed, decoder_blocks, decoder_norm, decoder_pred):
        x = decoder_embed(x)
...
#
    ...
    def forward(self, imgs, mask_ratio=0.5,vis=False):
        ...
        latent, feat6, feat8, feat10, feat12, active, ids_restore, active_hw = self.forward_encoder(imgs, mask_ratio)

        rec6 = self.forward_aux_decoder(
            feat6, ids_restore,
            self.aux_decoder_embed6, self.aux_mask_token6, self.aux_decoder_pos_embed6,
            self.aux_decoder_blocks6, self.aux_decoder_norm6, self.aux_decoder_pred6
        )

        rec8 = self.forward_aux_decoder(
            feat8, ids_restore,
            self.aux_decoder_embed8, self.aux_mask_token8, self.aux_decoder_pos_embed8,
            self.aux_decoder_blocks8, self.aux_decoder_norm8, self.aux_decoder_pred8
        )

        rec10 = self.forward_aux_decoder(
            feat10, ids_restore,
            self.aux_decoder_embed10, self.aux_mask_token10, self.aux_decoder_pos_embed10,
            self.aux_decoder_blocks10, self.aux_decoder_norm10, self.aux_decoder_pred10
        )
        ...
        total_loss = 0.7 * recon_loss + 0.1 * loss6 + 0.1 * loss8 + 0.1 * loss10
