...
class MS_Attention_Quadratic_LogPE(nn.Module):
    """
    Quadratic dot-product attention used in the v2 pretraining run:
      - quadratic Q @ K^T  (instead of linearized K^T @ V)
      - 2D Log-PE bias added to the NxN attention map
      - learnable scalar gate (pe_scale) on the PE bias, via softplus
    Q, K, V all use Multispike here (the LIF variant lives in
