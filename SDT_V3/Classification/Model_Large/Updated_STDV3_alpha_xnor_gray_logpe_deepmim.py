...
# ---------------------------------------------------------------------------
# Thesis model variant:  DeepMIM + Gray PE + Log PE + alpha-XNOR attention
#
#   * Base structure is the clean DeepMIM pretraining model
#     (Updated_STDV3_2.py). All DeepMIM behavior is preserved: the same
#     masking, the same primary decoder, the same 6th/8th/10th auxiliary
#     decoders and their loss weights, and the same forward output format
#     (returns `total_loss, recon`) expected by main_pretrain.py.
#
#   * Gray positional encoding (2D Gray-code, concatenated to spiking Q/K)
#     and 2D Log positional encoding (L1-distance bias on the NxN map) are
#     both kept exactly as in v2.
#
#   * The ONLY change vs. the v2 quadratic/XNOR model is the Q-K SIMILARITY:
#     the normal XNOR / dot-product score is replaced by the alpha-XNOR
#     similarity of Xiao et al., "Rethinking Spiking Self-Attention
#     Mechanism: Implementing alpha-XNOR Similarity Calculation in Spiking
#     Transformers", CVPR 2025.
#
#   * alpha-XNOR redefines matching non-spike (0-0) pairs to contribute a
#     small value `alpha` instead of 0, while matching spikes (1-1) keep a
#     contribution of 1 and mismatches contribute 0. See the heavily
#     commented block inside MS_Attention_AlphaXNOR_GRAY_LogPE for the exact
#     math and the matmul-based closed form actually used.
#
#   * `alpha` is configurable from the attention class (default 0.25) and is
#     threaded through the block / model / factory functions. Setting
#     alpha = 1.0 recovers the v2 XNOR model exactly; alpha = 0.0 recovers
#     plain dot-product attention. The CVPR paper reports its best results
#     around alpha = 0.2 - 0.3 on ImageNet.
#
# Notes on spiking nodes / gradient flow:
#   * Q/K spiking neurons (MultiStepLIFNode) and V (Multispike) are kept
#     IDENTICAL to the working v2 model. No new stateful nodes are added and
#     `functional.reset_net(self)` is still called at the start of forward,
#     so there is no graph/state reuse change relative to v2.
#   * The alpha-XNOR similarity uses only standard differentiable ops
#     (matmul, sum, scalar mul/add). No `.detach()`, no `torch.no_grad()`,
#     and no custom autograd Function are used, so the backward graph
#     behaves exactly like the v2 quadratic model.
# ===========================================================================
...
def _build_gray_pe_2d(H: int, W: int, num_bits: int) -> torch.Tensor:
    i_idx = torch.arange(H)
    j_idx = torch.arange(W)
    i_gray = i_idx ^ (i_idx >> 1)
    j_gray = j_idx ^ (j_idx >> 1)
    i_mat, j_mat = torch.meshgrid(i_idx, j_idx, indexing='ij')
    i_mat, j_mat = i_mat.flatten(), j_mat.flatten()
    bits = torch.arange(num_bits - 1, -1, -1)
    i_codes = ((i_gray[i_mat].unsqueeze(-1) >> bits) & 1).float()
    j_codes = ((j_gray[j_mat].unsqueeze(-1) >> bits) & 1).float()
    return torch.cat([i_codes, j_codes], dim=-1)  # (N, 2*num_bits)
...
# ===========================================================================
# alpha-XNOR attention + Gray PE + Log-PE
# ===========================================================================
class MS_Attention_AlphaXNOR_GRAY_LogPE(nn.Module):
    """
    alpha-XNOR Spiking Self-Attention (Xiao et al., CVPR 2025) on top of the
    v2 quadratic / Gray-PE / Log-PE attention. Everything except the Q-K
    similarity is kept identical to the v2 model:
      - quadratic (full NxN) attention map
      - 2D Gray-code positional encoding concatenated to spiking Q and K
      - 2D Log-PE bias added to the NxN map, gated by a learnable scalar
      - binary MultiStepLIFNode for Q/K, Multispike for V

    The Q-K similarity is the only thing that changes: the v2 XNOR /
    dot-product score is replaced by the alpha-XNOR similarity (see forward).
    """
    ...
    def __init__(
        ...
        self.gray_scale = nn.Parameter(torch.tensor(-4.0))
        self.pe_scale = nn.Parameter(torch.tensor(-4.0))
        ...
        self.gray_bits = math.ceil(math.log2(side))
        ...
        gray_pe = _build_gray_pe_2d(side, side, self.gray_bits)
        self.register_buffer("gray_pe", gray_pe)   # (N, 2*gray_bits)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        k2 = (k * k).sum(dim=-1, keepdim=True).transpose(-2, -1)

        attn_spike = (1.0 + alpha) * qk - alpha * q2 - alpha * k2 + alpha * D_spike


        gray = self.gray_pe.to(q.device).to(q.dtype)   # (N, Dg)

        D_gray = gray.size(-1)
        gray_sum_i = gray.sum(dim=-1, keepdim=True)       # (N, 1)
        gray_sum_j = gray.sum(dim=-1, keepdim=True).T     # (1, N)
        gray_dot = torch.matmul(gray, gray.T)             # (N, N)

        gray_xnor = D_gray - gray_sum_i - gray_sum_j + 2.0 * gray_dot
        gray_xnor = gray_xnor.view(1, 1, 1, N, N)
        ...
        # Estimate firing/statistics without backpropagating through the statistics.
        q_stat = q.detach()
        k_stat = k.detach()
        v_stat = v.detach()

        fQ = q_stat.mean().clamp(eps, 1.0 - eps)
        fK = k_stat.mean().clamp(eps, 1.0 - eps)


        v_second = (v_stat * v_stat).mean().clamp_min(eps)

        alpha_t = q.new_tensor(alpha)

        g1 = fQ * fK + alpha_t * (1.0 - fQ) * (1.0 - fK)
        g2 = fQ * fK + (alpha_t ** 2) * (1.0 - fQ) * (1.0 - fK)
        var = (g2 - g1 * g1).clamp_min(eps)

        k_lin = torch.rsqrt(N * D_spike * var * v_second)
        b_lin = -k_lin * D_spike * g1

        attn_content = k_lin * attn_spike + b_lin

        gray_bias = gray_xnor - gray_xnor.mean(dim=-1, keepdim=True)

        log_bias = self.log_pe.to(q.device).to(q.dtype)
        log_bias = log_bias - log_bias.mean(dim=-1, keepdim=True)

        attn = (
            attn_content
            + F.softplus(self.gray_scale) * gray_bias
            + F.softplus(self.pe_scale) * log_bias
            )
...
class MS_Block(nn.Module):
    def __init__(
        ...
        if attn_type == "quadratic_logpe":
            self.attn = MS_Attention_AlphaXNOR_GRAY_LogPE(
                dim=dim,
                num_heads=num_heads,
                sr_ratio=sr_ratio,
...
# ===========================================================================
# DeepMIM + SpikMAE encoder-decoder with alpha-XNOR + Gray-PE + Log-PE
# ===========================================================================
...
class SpikmaeDeepMIM(nn.Module):
    def __init__(
        ...
        # alpha-XNOR weight (passed down to the attention blocks)
        self.alpha = alpha
...
if __name__ == "__main__":
    ...
    model = spikmae_deepmim_12_768(alpha=0.25)
    print(f"alpha-XNOR alpha = {model.alpha}")
    x = torch.randn(2, 3, 224, 224)
    total_loss, recon = model(x, mask_ratio=0.50)
    print(f"total={total_loss.item():.4f}, recon={recon.item():.4f} ✓")
