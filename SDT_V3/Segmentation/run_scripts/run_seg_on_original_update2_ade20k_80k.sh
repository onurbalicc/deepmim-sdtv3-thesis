...
# Model: Update 2
# DeepMIM + Log-PE 2D + Quadratic Q,K LIF
# Metric: mIoU
# Schedule: 80k iterations, validation every 8k iterations
# ============================================================
...
CKPT="../Classification/Model_Large/09_updated_stdv3_v2_debug_pretrain_2gpu_400ep/checkpoint-390.pth"
WORK_DIR="work_dirs/05_seg_on_original_update2_ade20k_80k"

GPUS=2
PORT="${PORT:-29533}"
GPUS_TO_USE="${GPUS_TO_USE:-5,6}"

echo "=== ADE20K Segmentation: Update 2 on ORIGINAL Spikformer ==="
...
CUDA_VISIBLE_DEVICES="$GPUS_TO_USE" PORT="$PORT" bash tools/dist_train.sh "$CONFIG" "$GPUS" \
  --work-dir "$WORK_DIR" \
  --cfg-options model.backbone.init_cfg.checkpoint="$CKPT"
